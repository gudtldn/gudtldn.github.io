...
## 5. 실전 활용 팁 및 자주 헷갈리는 개념 정리

### 5.1. 스플라이서(`[: ... :]`) vs 값 추출(`extract<T>`)

리플렉션 정보(`std::meta::info`)를 다시 일반 C++ 코드로 되돌린다는 점에서 두 기능은 비슷해 보이지만, **언어 문법(연산자)**인지 **라이브러리(함수)**인지에 따른 결정적인 차이가 있습니다.

| 구분          | 스플라이서 `[: r :]`                           | 값 추출 `extract<T>(r)`                           |
| :------------ | :--------------------------------------------- | :------------------------------------------------ |
| **정의**      | C++ 핵심 언어 **연산자 (Operator)**            | `<meta>` 헤더의 **라이브러리 함수 (`consteval`)** |
| **반환 결과** | 식별자, 타입명, 표현식 그 자체 (**코드 주입**) | 컴파일 타임에 평가된 **순수한 상수값 (Value)**    |
| **동작 비유** | 매크로나 코드 붙여넣기 (Xvalue)                | `constexpr` 함수의 반환값 (Rvalue)                |
| **주요 목적** | 메타데이터를 소스코드 자리에 **그대로 치환**   | 리플렉션된 상수의 **값 자체를 추출하여 연산**     |

#### 코드 비교로 이해하기

```c++
constexpr int my_value = 42;
constexpr std::meta::info r = ^^my_value;

// 1. Splicer: 문법적으로 변수/식 자체를 그 자리에 대치
int a = [:r:]; // int a = my_value;
[:r:] = 10;    // my_value = 10;

// 2. Extract: 순수한 "값(42)"을 컴파일 타임에 평가하여 추출
int b = std::meta::extract<int>(r); // int b = 42;
```
