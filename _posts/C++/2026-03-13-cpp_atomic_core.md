...
{: .prompt-info }
> 가짜 실패란, CAS 연산이 실제로는 성공할 조건(값이 일치함)임에도 불구하고, 하드웨어적인 이유로 인해 실패를 반환하는 현상을 말합니다.
>
> x86 아키텍처는 단일 명령(`cmpxchg`)을 지원하므로 가짜 실패가 없습니다. 하지만 **ARM/RISC-V 등은 LL/SC(Load-Link/Store-Conditional) 명령어 쌍**을 조합하여 CAS를 구현합니다. 이 구조에서는 메모리를 읽고 수정하는 사이에 **컨텍스트 스위칭, 인터럽트, 캐시 라인 교체 등**의 하드웨어 이벤트가 발생하면 주소 모니터링 상태가 강제로 해제되면서 가짜 실패가 발생하게 됩니다.

| 종류     | 가짜 실패(Spurious Failure) 발생 여부 | 특징                                                                                 |
| -------- | ------------------------------------- | ------------------------------------------------------------------------------------ |
| `strong` | 발생하지 않음                         | 값이 같으면 무조건 성공 보장. 단일 조건문이나 직관적인 로직에 적합.                  |
| `weak`   | **발생 가능**                         | 값이 같아도 하드웨어 메커니즘에 의해 실패할 수 있음. 반드시 반복문 내에서 사용 권장. |
...
{: .prompt-warning }
> 주의: `failure_order`는 값을 수정하지 않으므로 `release`나 `acq_rel`은 사용할 수 없습니다.
